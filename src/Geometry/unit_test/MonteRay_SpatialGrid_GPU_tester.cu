...
//		CHECK(false);
        ...
        CHECK_EQUAL( 100, getNumCells() );
    }

	TEST_FIXTURE(SpatialGridGPUTester, getDimension_on_GPU ) {
		sphericalGrid1_setup();
        CHECK_EQUAL( 1, getDimension() );
    }

	TEST_FIXTURE(SpatialGridGPUTester, getCoordinateSystem_on_GPU ) {
		sphericalGrid1_setup();
        CHECK_EQUAL( TransportMeshTypeEnum::Spherical, getCoordinateSystem() );
    }
	...
	TEST_FIXTURE(SpatialGridGPUTester, getVertex ) {
		sphericalGrid1_setup();
        CHECK_CLOSE( 0.1, getVertex(0,0), 1e-4 ); // 0 vertex is removed by MonteRay_GridBins::modifyForRadial
        CHECK_CLOSE( 0.2, getVertex(0,1), 1e-4 );
    }
	...
	TEST_FIXTURE(SpatialGridGPUTester, getIndex ) {
		...
		unsigned outside = OUTSIDE_INDEX;
		CHECK_EQUAL( outside, getIndex( Position_t(-9999.9,-19.9,-29.9) ) ); // outside neg-x
        CHECK_EQUAL( 0, getIndex( Position_t(-0.01, -0.01, -0.01) ) ); // first index
        CHECK_EQUAL( 1, getIndex( Position_t(  0.1,   0.1,   0.1) ) ); // second index
        CHECK_EQUAL(99, getIndex( Position_t( 9.95,   0.0,   0.0) ) ); // last index
    }
	...
	TEST_FIXTURE(SpatialGridGPUTester, getIndex_via_particle ) {
		...
		p.pos =  Position_t(-0.01, -0.01, -0.01);
        CHECK_EQUAL( 0, getIndex( p ) ); // first index

        p.pos =  Position_t(0.1, 0.1, 0.1);
        CHECK_EQUAL( 1, getIndex( p ) ); // second index

        p.pos =  Position_t(9.95, 0.0, 0.0);
