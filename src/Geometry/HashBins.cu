...
HashBins::HashBins(gpuFloatType_t *vertices, unsigned nVertices, unsigned nHashBinEdges) : CopyMemoryBase() {
    ...
    reallocate(vertices, nVertices, nHashBinEdges);
}

void
HashBins::reallocate(gpuFloatType_t *vertices, unsigned nVertices, unsigned nHashBinEdges) {
    MONTERAY_ASSERT( nHashBinEdges > 1 );
...
void HashBins::writeToFile( const std::string& filename ) {
    std::ofstream outfile;

    outfile.open( filename.c_str(), std::ios::binary | std::ios::out);
    if( ! outfile.is_open() ) {
        fprintf(stderr, "HashBins::writeToFile -- Failure to open file,  filename=%s  %s %d\n", filename.c_str(), __FILE__, __LINE__);
        throw std::runtime_error("HashBins::writeToFile -- Failure to open file" );
    }
    assert( outfile.good() );
...
void HashBins::readFromFile( const std::string& filename ) {
    std::ifstream infile;
    if( infile.is_open() ) {
        infile.close();
    }
    ...
    if( ! infile.is_open() ) {
        fprintf(stderr, "Error:  HashBins::readFromFile -- Failure to open file,  filename=%s  %s %d\n", filename.c_str(), __FILE__, __LINE__);
        throw std::runtime_error("HashBins::readFromFile -- Failure to open file" );
    }
    assert( infile.good() );
...
void
HashBins::write(std::ostream& outf) const {
    unsigned version = 0;
    binaryIO::write(outf, version );

    binaryIO::write(outf, min);
    binaryIO::write(outf, max);
    binaryIO::write(outf, invDelta);
    binaryIO::write(outf, nEdges);

    for( unsigned i = 0; i<nEdges; ++i) {
        binaryIO::write(outf, binBounds[i]);
    }

}

void
HashBins::read(std::istream& infile) {
    unsigned version;
    binaryIO::read(infile, version );

    binaryIO::read(infile, min);
    binaryIO::read(infile, max);
    binaryIO::read(infile, invDelta);
    binaryIO::read(infile, nEdges);
    ...
    for( unsigned i = 0; i<nEdges; ++i) {
        binaryIO::read(infile, binBounds[i]);
    }
}
