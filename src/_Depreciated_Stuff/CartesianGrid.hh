...
public:
	...
	unsigned rayTrace( int* global_indices, float_t* distances, const Position_t& pos, const Position_t& dir, float_t distance,  bool outsideDistances) const{
		...
		if(debug) std::cout << "CartesianGrid::rayTrace --\n";

	    int current_indices[3] = {0, 0, 0}; // current position indices in the grid, must be int because can be negative if outside
	    ...
	    for( unsigned i=0; i<3; ++i){
	    	...
	    	if(debug) std::cout << "CartesianGrid::rayTrace -- i=" << i << " numCrossings[i]=" << numCrossings[i] << "\n";
	...
	// vertices are statically allocated so that we can attempt using CONSTANT memory
